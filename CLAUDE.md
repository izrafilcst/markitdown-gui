# Ruflo, configuracao do Claude Code
...
1. **Nada de travessao americano.** Proibido o caractere de travessao longo
   ...
   copy da interface e mensagem de commit. Use virgula, dois-pontos, ponto
   ou parenteses.
2. **`markitdown_gui/core/` nunca importa Qt.** O nucleo e Python puro, e e
   por isso que a logica e testavel sem abrir janela.
3. **Zero rede.** Nenhum `requests`, `urllib`, `httpx`, `aiohttp`, chave de
   API ou endpoint. O app promete funcionar offline e a promessa e
   verificada por `tests/test_offline.py`.
4. **Identificadores de codigo sem acento.** Strings visiveis ao usuario e
   documentacao levam acento normalmente.
5. **Cor literal em hexadecimal so existe em `markitdown_gui/ui/theme.py`.**
   Qualquer outro arquivo importa de `theme.COLOR`.
...
- ALWAYS name agents, because `name: "role"` makes them addressable
